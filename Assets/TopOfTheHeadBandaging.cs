...
public class TopOfTheHeadBandaging : MonoBehaviour // 3steps only
{
	...
	[SerializeField] bool TwoFingerDone = false;
	...
	void AnimationClipController()
	{
		...
		if (BandageStep > 0)
		{
			...
			{
				StepsAnimation.SetInteger("Steps", StepCounts);
				if (_bandage[0].IsSuccess == true && _bandage[1].IsSuccess == true)
				{
				...
				if (_bandage[2].IsSuccess == true && _bandage[3].IsSuccess == true)
				{
			...
			else if (TwoFingerDone == true)
			{
				...
				if (_bandage[23].IsSuccess == true)
				{
					Steps[18].SetActive(false);
					StepCounts = 19;
					BandageStep = 1;
				}
				if (_bandage[24].IsSuccess == true)
				{
